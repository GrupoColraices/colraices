  ...
  return (
    <section className="bg-[rgba(255,193,7,0.10)] px-4 py-12 md:px-6 md:py-16 lg:py-[88px]">
      <div className="mx-auto mb-8 w-full max-w-[1180px] text-center md:mb-12 lg:px-[52px]">
        <div className="mb-2 text-xs font-bold uppercase tracking-[0.12em] text-[#FFC107] md:text-sm">
          Proceso
        </div>
      ...
      <div className="mx-auto grid w-full max-w-[1180px] grid-cols-[auto_1fr_auto] items-center gap-2 sm:gap-4 md:gap-5">
        <button
          ...
          aria-label="Anterior"
          onClick={prev}
          className="grid h-9 w-9 place-items-center rounded-full border border-[#FFC107] text-2xl leading-none text-[#FFC107] disabled:opacity-50"
        ...
        <div ref={viewportRef} className="overflow-hidden">
          ...
          >
            {looped.map((step, idx) => (
              ...
              >
                <div className="mb-4 grid h-10 w-10 place-items-center rounded-full bg-[#FFC107] text-2xl font-bold text-white md:h-11 md:w-11 md:text-3xl">
                  {step.id}
                </div>
                ...
                <span className="inline-block rounded-full bg-[rgba(255,193,7,0.12)] px-3 py-1 text-[10.88px] font-semibold leading-[16.3px] text-[#FFC107]">
                  {step.time}
                </span>
              </article>
            ))}
      ...
      <div className="mt-5 text-center" aria-hidden="true">
        {steps.map((_, i) => (
          <span
            key={i}
            className={`mx-1 inline-block h-2 rounded-full ${i === realIndex ? "w-[22px] bg-[#FFC107]" : "w-2 bg-[rgba(255,193,7,0.12)]"}`}
          />
        ))}
      </div>
    </section>
